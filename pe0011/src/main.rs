fn main() {
    ...
    println!("{:?}", the_thing(rows));
}

fn the_thing(matrix: Vec<Vec<u32>>) -> u32{
    let mut largest = 0;
    for i in 0..=19{
        for j in 0..=19{
            //println!("i:{:?}, j:{}", i, j);
            if j+3<=19 {
                let candidate = matrix[i][j]*matrix[i][j+1]*matrix[i][j+2]*matrix[i][j+3];
                if largest < candidate {largest = candidate}
            }
            //println!("Here1");
            ...
            if i+3 <=19{
                let candidate = matrix[i][j]*matrix[i+1][j]*matrix[i+2][j]*matrix[i+3][j];
                if largest < candidate {largest = candidate}
            }
            //println!("Here3");
            ...
            if j+3<=19 && i+3<=19 {
                let candidate = matrix[i][j]*matrix[i+1][j+1]*matrix[i+2][j+2]*matrix[i+3][j+3];
                if largest < candidate {largest = candidate}
            }
            //println!("Here");

            if j.checked_sub(3).is_some() && i.checked_sub(3).is_some() {
                let candidate = matrix[i][j]*matrix[i-1][j-1]*matrix[i-2][j-2]*matrix[i-3][j-3];
                if largest < candidate {largest = candidate}
            }
            //println!("Here");

            if j+3<=19 && i.checked_sub(3).is_some() {
                let candidate = matrix[i][j]*matrix[i-1][j+1]*matrix[i-2][j+2]*matrix[i-3][j+3];
                if largest < candidate {largest = candidate}
            }
            //println!("Here");

            if j.checked_sub(3).is_some() && i+3<=19 {
                let candidate = matrix[i][j]*matrix[i+1][j-1]*matrix[i+2][j-2]*matrix[i+3][j-3];
                if largest < candidate {largest = candidate}
            }
        }
